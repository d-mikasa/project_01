...
$message =
    ...
    'ご予約いただいた内容をお知らせします。'. "\n" .
    "\n" .
    '宿泊代表者氏名：' . h($insert_data['user_name']) . ' 様'. "\n" .
    '宿名：CICACU'. "\n" .
    '電話番号：080-1411-4095'. "\n" .
    '所在地：〒322-0067 栃木県鹿沼市天神町1704'. "\n" .
    'チェックイン日時：' . h($_POST['check_in']) .''. "\n" .
    'チェックアウト日時：' . h($_POST['check_out']) . ''. "\n" .
    "\n" .
    '部屋タイプ：' . h($insert_data['name']) . ''. "\n" .
    "\n" .
    ...
    '-----------------------------------------------------------------------'. "\n" .
    '【料金明細】'. "\n" .
    '料金            ：' . h($insert_data['price']) . ' 円× ' . h($_POST['capacity']) . ' 人'. "\n" .
    '宿泊日数    ：' . h($insert_data['total_stay']) . '日'. "\n" .
    '合計            ：' . h($insert_data['total_price']) . ' 円（税込・サービス料別）';
